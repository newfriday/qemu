...
struct QCryptoGMT {
    ...
    SGD_HANDLE session;    /* thread-safe for SWXA-specific implementation */
    SGD_HANDLE key;        /* thread-unsafe for SWXA-specific implementation */
    SGD_UINT32 alg;
...
/*
 * For the SWXA-specific implementation of GMT devices. It is advised to
 * to use numerous sessions with one open device as opposed to single
 * sessions with several devices.
 */
                           ...
                           Error **errp)
{
    ...
    if (!(gmt_device.info.SymAlgAbility & SGD_SM4_ECB & SGD_SYMM_ALG_MASK)) {
        ...
        info_report("Encryption card do not support SM4 cipher algorithm, disabling");
        return NULL;
    }

    rv = SDF_ImportKey(gmt->session, (SGD_UCHAR *)key, (SGD_UINT32)nkey, &gmt->key);
    if (rv != SDR_OK) {
