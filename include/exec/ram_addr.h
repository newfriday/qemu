                                               ...
                                               RAMBlockSyncMode mode)
{
    ...
    if (rb->background_sync_shown_up &&
        (mode == RAMBLOCK_SYNC_BG || mode == RAMBLOCK_SYNC_ITER)) {
    ...
    if (((word * BITS_PER_LONG) << TARGET_PAGE_BITS) ==
        ...
        for (k = page; k < page + nr; k++) {
            if (src[idx][offset]) {
                ...
                if (mode == RAMBLOCK_SYNC_BG || mode == RAMBLOCK_SYNC_ITER) {
                    /* Back-up bmap for the next iteration */
                    iter_bmap[k] |= bits;
                    if (mode == RAMBLOCK_SYNC_BG) {
